...
public abstract class AbstractPasswordLibraryLoader
    implements IPasswordLibraryLoader {
    ...
    public IPasswordLibrary load(
        ...
        String decryptedXMLContent = decryptXMLContent(
            encryptedXMLContent,
            masterPassword
        );
    ...
    protected String loadFromFile(File file) throws FileNotFoundException {
        ...
        if (file == null || !file.exists()) {
            throw new FileNotFoundException(
                "File " + file.getPath() + " could not be found."
            );
        }
        ...
        try {
            ...
            e.printStackTrace();
            throw new FileNotFoundException(
                "File " + file.getPath() + " could not be read."
            );
        }
    }

    protected abstract String decryptXMLContent(
        String encryptedXMLContent,
        String masterPassword
    ) throws DecryptionFailedException;

    protected abstract void decodeFromXML(
        String xmlContent,
        IPasswordLibrary passwordLibrary
    );
}
