...
import eu.portunus.PortunusFactory;
import eu.portunus.controller.util.OverviewController;
...
public abstract class AbstractPortunus extends Application {
    ...
    private static File passwordLibraryFile = new File(
        "PasswordLibrary.portunus"
    );
    ...
    protected IPasswordLibrary doLoadPasswordLibrary()
        ...
        if (passwordLibrary == null) {
            ErrorUtil.showFactoryMethodError(
                IPasswordLibrary.class,
        ...
        if (loader == null) {
            ErrorUtil.showFactoryMethodError(
                IPasswordLibraryLoader.class,
                "Password library will be empty."
            );
            return passwordLibrary;
        }
    ...
    private void savePasswordLibrary() {
        ...
        if (saver == null) {
            ErrorUtil.showFactoryMethodError(
                IPasswordLibrarySaver.class,
                "Password library will not be saved."
            );
            return;
        }
        ...
        try {
            ...
            ErrorUtil.showError(
                "Encryption of password library failed on save."
            );
        }
    }
