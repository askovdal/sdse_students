...
import eu.portunus.view.OverviewPane;
import eu.portunus.view.contextmenu.PasswordEntryContainerTreeViewContextMenu;
import eu.portunus.view.contextmenu.PasswordRecordTableViewContextMenu;
import eu.portunus.view.control.PasswordEntryContainerTreeItem;
import eu.portunus.view.control.PasswordEntryContainerTreeView;
import eu.portunus.view.control.PasswordRecordTableView;
import eu.portunus.view.dialog.AddPasswordGroupDialog;
import eu.portunus.view.dialog.AddPasswordRecordDialog;
import eu.portunus.view.dialog.EditPasswordGroupDialog;
import eu.portunus.view.dialog.EditPasswordRecordDialog;
import eu.portunus.view.util.ErrorUtil;
...
public class OverviewController {
    ...
    protected void registerListeners() {
        ...
        passwordEntryContainerTreeView
            ...
            .addListener(
                new ChangeListener<TreeItem<IPasswordEntryContainer>>() {
                    @Override
                    public void changed(
                        ObservableValue<? extends TreeItem<IPasswordEntryContainer>> observable,
                        TreeItem<IPasswordEntryContainer> oldValue,
                        TreeItem<IPasswordEntryContainer> newValue
                    ) {
    ...
    protected void initializeChildControllers() {
        ...
        PasswordRecordTableViewContextMenuController passwordRecordTableViewContextMenuController = new PasswordRecordTableViewContextMenuController(
            passwordRecordContextMenu,
            this
        );
        ...
        PasswordEntryContainerTreeViewContextMenuController passwordEntryContainerTreeViewContextMenuController = new PasswordEntryContainerTreeViewContextMenuController(
            passwordEntryContainerTreeViewContextMenu,
            this
        );
    ...
    public void addPasswordGroup(IPasswordEntryContainer parentContainer) {
        ...
        AddPasswordGroupDialog dialog = new AddPasswordGroupDialog(
            passwordGroup
        );
    ...
    public void editPasswordGroup(IPasswordGroup passwordGroup) {
        ...
        EditPasswordGroupDialog dialog = new EditPasswordGroupDialog(
            passwordGroup
        );
        Optional<IPasswordGroup> result = dialog.showAndWait();
    ...
    public void deletePasswordGroup(IPasswordGroup passwordGroup) {
        boolean reallyDelete = showDeleteConfirmationDialog(
            "group",
            passwordGroup.getTitle()
        );
    ...
    public void addPasswordRecord() {
        IPasswordRecord passwordRecord = factory.createPasswordRecord(
            "New Record"
        );
        ...
        AddPasswordRecordDialog dialog = new AddPasswordRecordDialog(
            passwordRecord,
            factory
        );
    ...
    public void editPasswordRecord(IPasswordRecord passwordRecord) {
        ...
        EditPasswordRecordDialog dialog = new EditPasswordRecordDialog(
            passwordRecord,
            factory
        );
        Optional<IPasswordRecord> result = dialog.showAndWait();
    ...
    protected boolean showDeleteConfirmationDialog(
    ...
    protected void doAddPasswordGroup(
        IPasswordGroup passwordGroup,
        IPasswordEntryContainer parentContainer
    ) {
    ...
    protected void doDeletePasswordGroup(IPasswordGroup passwordGroup) {
        IPasswordEntryContainer container = findPasswordEntryContainer(
            passwordGroup
        );
    ...
    private IPasswordEntryContainer findPasswordEntryContainer(
        IPasswordEntry searchedEntry
    ) {
    ...
    private IPasswordEntryContainer findPasswordEntryContainer(
        IPasswordEntry searchedEntry,
        IPasswordEntryContainer currentContainer
    ) {
        ...
        for (IPasswordEntry childEntry : childEntries) {
            if (childEntry instanceof IPasswordEntryContainer) {
                ...
                IPasswordEntryContainer foundContainer = findPasswordEntryContainer(
                    searchedEntry,
                    childContainer
                );
    ...
    private TreeItem<IPasswordEntryContainer> findPasswordEntryContainerTreeItem(
        IPasswordEntryContainer searchedPasswordEntryContainer
    ) {
        PasswordEntryContainerTreeView passwordEntryContainerTreeView = overviewPane.getPasswordEntryContainerTreeView();

        TreeItem<IPasswordEntryContainer> rootTreeItem = passwordEntryContainerTreeView.getRoot();

        return findPasswordEntryContainerTreeItem(
            searchedPasswordEntryContainer,
            rootTreeItem
        );
    }
    ...
    private TreeItem<IPasswordEntryContainer> findPasswordEntryContainerTreeItem(
        IPasswordEntryContainer searchedPasswordEntryContainer,
        TreeItem<IPasswordEntryContainer> currentTreeItem
    ) {
        ...
        for (TreeItem<IPasswordEntryContainer> childTreeItem : childTreeItems) {
            TreeItem<IPasswordEntryContainer> foundTreeItem = findPasswordEntryContainerTreeItem(
                searchedPasswordEntryContainer,
                childTreeItem
            );
    ...
    private void doSearchPasswordEntries(String partialString) {
        ...
        overviewPane
            .getPasswordEntryContainerTreeView()
            .getSelectionModel()
            .clearSelection();

        //Find matching password records
        List<IPasswordRecord> matchingPasswordRecords = finder.findMatchingPasswordRecords(
            partialString,
            passwordLibrary
        );
    ...
    protected void selectPasswordGroup(IPasswordGroup passwordGroup) {
        TreeItem<IPasswordEntryContainer> treeItem = findPasswordEntryContainerTreeItem(
            passwordGroup
        );
    ...
    public IPasswordGroup getSelectedPasswordGroup() {
        PasswordEntryContainerTreeView passwordEntryContainerTreeView = overviewPane.getPasswordEntryContainerTreeView();
        TreeItem<IPasswordEntryContainer> selectedPasswordEntryContainerTreeItem = passwordEntryContainerTreeView
            .getSelectionModel()
            .getSelectedItem();
    ...
    private void updatePasswordEntryContainerTreeView() {
        ...
        PasswordEntryContainerTreeItem rootTreeItem = createPasswordEntryTreeViewItem(
            passwordLibrary
        );
    ...
    private PasswordEntryContainerTreeItem createPasswordEntryTreeViewItem(
        IPasswordEntryContainer passwordEntryContainer
    ) {
        PasswordEntryContainerTreeItem treeViewItem = new PasswordEntryContainerTreeItem(
        ...
        for (IPasswordEntry entry : entries) {
            if (entry instanceof IPasswordGroup) {
                ...
                PasswordEntryContainerTreeItem childTreeViewItem = createPasswordEntryTreeViewItem(
                    childPasswordGroup
                );
    ...
    private void updatePasswordRecordTableView(
        IPasswordGroup selectedPasswordGroup
    ) {
        if (selectedPasswordGroup != null) {
